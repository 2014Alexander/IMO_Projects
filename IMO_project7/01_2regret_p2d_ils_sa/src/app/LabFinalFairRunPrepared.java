...
import faircomparison.LabFinalAlgorithmDefinition;
...
/**
 * Final lab fair benchmark harness.
 *
 * Fair execution order:
 *   for each instance
 *     for each run-config (startVertex, runSeed)
 *       for each algorithm
 *         create fresh algorithm instance and run it once
 *
 * This avoids running all 200 starts for one algorithm before the next algorithm.
 */
public final class LabFinalFairRunPrepared {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<LabFinalAlgorithmDefinition> algorithms = LabFinalAlgorithmDefinition.defaultAlgorithms();
        ...
        for (String instancePath : instancePaths) {
            ...
            for (FairRunConfig config : configs) {
                for (int algorithmIndex = 0; algorithmIndex < algorithms.size(); algorithmIndex++) {
                    LabFinalAlgorithmDefinition definition = algorithms.get(algorithmIndex);
    ...
    private static void printHeader() {
        System.out.println("instance,runIndex,startVertexId,runSeed,algorithmIndex,algorithm,timeLimited,objective,profitSum,tourLength,cycleSize,runtimeNanos,iterationCount,acceptedBetterCount,acceptedWorseCount,rejectedWorseCount,bestFoundIteration,timeLimitSeconds");
    }
