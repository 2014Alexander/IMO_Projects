...
public final class NextAlgorithmSingleVariantRun {
    ...
    public static void main(String[] args) {
        Kind selectedKind = Kind.valueOf(args[0]);
        int runsCount = args.length >= 2 ? Integer.parseInt(args[1]) : DEFAULT_RUNS_COUNT;
        long baseSeed = args.length >= 3 ? Long.parseLong(args[2]) : DEFAULT_BASE_SEED;
        String[] instancePaths = args.length >= 4 ? instancePathsFromOffset(args, 3) : new String[] {"data/TSPA.csv", "data/TSPB.csv"};
        ...
        for (String instancePath : instancePaths) {
            ...
            runVariant(instance, runs, selected);
        }
    }
    ...
    private static String[] instancePathsFromOffset(String[] args, int offset) {
