 ...
 * </ol>
 */
public final class CheapConsensusR2Top3P30Parameterized implements OptimizationAlgorithm {
    ...
    private final boolean useFastRandomized;
    private final Random random;
    private final PhaseTwoDelete phaseTwoDelete;

    public CheapConsensusR2Top3P30Parameterized(long seed, int lsCandidates, int eliteSize, boolean useBestParentFallback) {
        this(new Random(seed), 10, lsCandidates, eliteSize, useBestParentFallback, false);
    }

    public CheapConsensusR2Top3P30Parameterized(long seed, int lsCandidates, int eliteSize, boolean useBestParentFallback, boolean useFastRandomized) {
        this(new Random(seed), 10, lsCandidates, eliteSize, useBestParentFallback, useFastRandomized);
    }

    public CheapConsensusR2Top3P30Parameterized(Random random, int baseRuns, int lsCandidates, int eliteSize, boolean useBestParentFallback) {
        this(random, baseRuns, lsCandidates, eliteSize, useBestParentFallback, false);
    }

    public CheapConsensusR2Top3P30Parameterized(Random random, int baseRuns, int lsCandidates, int eliteSize, boolean useBestParentFallback, boolean useFastRandomized) {
        this.random = Objects.requireNonNull(random, "random");
        this.baseRuns = baseRuns;
        this.lsCandidates = lsCandidates;
        this.eliteSize = eliteSize;
        this.useBestParentFallback = useBestParentFallback;
        this.useFastRandomized = useFastRandomized;
        this.phaseTwoDelete = new PhaseTwoDelete();
    }

    @Override
    public String name() {
        return "CheapConsensus" + (useFastRandomized ? "Fast" : "") + "R2Top3P30_P2DAll10_LSTop" + lsCandidates + "_Top" + eliteSize + (useBestParentFallback ? "_OrBestParent" : "");
    }

    @Override
    public Solution solve(Instance instance, int startVertexId) {
        List<ScoredSolution> base = new ArrayList<>(baseRuns);

        for (int run = 0; run < baseRuns; run++) {
            OptimizationAlgorithm construction = useFastRandomized
                    ? new FastRandomized2RegretParameterizedWithPhaseTwoDelete(
                            "FAST_R2_TOP3P30_CHEAP_CONSENSUS_try" + run,
                            3,
                            0.30,
                            new Random(random.nextLong())
                    )
                    : new Randomized2RegretParameterizedWithPhaseTwoDelete(
