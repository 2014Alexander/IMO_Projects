...
public final class R2RegretPotentialGridRun {
    ...
    private record Variant(String name, int topK, double probability, int triesCount, boolean baseline) {}
    ...
    public static void main(String[] args) {
        ...
        System.out.println("instance,algorithm,runIndex,startVertexId,seed,objective,runtimeNanos,iterationCount,acceptedBetterCount,acceptedWorseCount,rejectedWorseCount,bestFoundIteration,topK,probability,triesCount,cooling,T0,Tmin");
        System.out.flush();
    ...
    private static List<Variant> variants() {
        ...
        result.add(new Variant("ILS_2REGRET_P2D_START_SA_GEO_T0_300_TMIN_10", 0, 0.0, 0, true));
        result.add(new Variant("ILS_R2REGRET_TOP3P25_BEST12_P2D_LS_START_SA_GEO_T0_300_TMIN_10", 3, 0.25, 12, false));
        result.add(new Variant("ILS_R2REGRET_TOP3P30_BEST8_P2D_LS_START_SA_GEO_T0_300_TMIN_10", 3, 0.30, 8, false));
        result.add(new Variant("ILS_R2REGRET_TOP3P30_BEST10_P2D_LS_START_SA_GEO_T0_300_TMIN_10", 3, 0.30, 10, false));
        result.add(new Variant("ILS_R2REGRET_TOP3P30_BEST12_P2D_LS_START_SA_GEO_T0_300_TMIN_10", 3, 0.30, 12, false));
        result.add(new Variant("ILS_R2REGRET_TOP3P30_BEST15_P2D_LS_START_SA_GEO_T0_300_TMIN_10", 3, 0.30, 15, false));
        return result;
    }
    ...
    private static void runVariant(Instance instance, List<RunConfig> runs, Variant variant) {
        for (int runIndex = 0; runIndex < runs.size(); runIndex++) {
            ...
            System.out.println(instance.name + "," + algorithm.name() + "," + runIndex + "," + run.startVertexId() + "," + run.runSeed() + "," + objective + "," + runtime + "," + iterationCount + "," + stats.acceptedBetterCount() + "," + stats.acceptedWorseCount() + "," + stats.rejectedWorseCount() + "," + stats.bestFoundIteration() + "," + variant.topK() + "," + variant.probability() + "," + variant.triesCount() + "," + COOLING + "," + T0 + "," + TMIN);
            System.out.flush();
        }
    }
    ...
    private static OptimizationAlgorithm createAlgorithm(Variant variant, long seed) {
        if (variant.baseline()) {
            return new IteratedLocalSearchWithTwoRegretStartAndSaAcceptance(variant.name(), new SteepestLocalSearchWithCandidateMoves(), new RandomSwapEdgesPerturbation(30), TIME_LIMIT_NANOS, seed, T0, TMIN, COOLING);
        }
        return new IteratedLocalSearchWithConstructionStartAndSaAcceptance(
            variant.name(),
            new Randomized2RegretParamBestNWithPhaseTwoDeleteStart(variant.name() + "_START", variant.topK(), variant.probability(), variant.triesCount(), seed, new SteepestLocalSearchWithCandidateMoves()),
            new SteepestLocalSearchWithCandidateMoves(), new RandomSwapEdgesPerturbation(30), TIME_LIMIT_NANOS, seed, T0, TMIN, COOLING
        );
    }
