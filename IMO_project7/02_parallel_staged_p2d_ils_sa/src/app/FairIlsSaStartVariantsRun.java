...
import faircomparison.IlsSaStartVariantDefinition;
...
/**
 * Fair comparison of ILS-SA variants that differ only by the start construction.
 * Execution order is run-config first, then algorithm, to avoid giving one algorithm a warmer JVM pass.
 */
public final class FairIlsSaStartVariantsRun {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<IlsSaStartVariantDefinition> algorithms = IlsSaStartVariantDefinition.defaultAlgorithms();
        ...
        for (String instancePath : instancePaths) {
            ...
            for (FairRunConfig config : configs) {
                for (int algorithmIndex = 0; algorithmIndex < algorithms.size(); algorithmIndex++) {
                    IlsSaStartVariantDefinition definition = algorithms.get(algorithmIndex);
