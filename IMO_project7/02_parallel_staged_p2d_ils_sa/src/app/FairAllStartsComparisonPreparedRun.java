...
import faircomparison.FairAlgorithmDefinition;
...
/**
 * Fair all-starts comparison harness.
 *
 * It executes by run-config first, then by algorithm:
 * for one shared (startVertex, seed) config, every tested algorithm is created independently and executed once.
 */
public final class FairAllStartsComparisonPreparedRun {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<FairAlgorithmDefinition> algorithms = FairAlgorithmDefinition.defaultAlgorithms();
        ...
        for (String instancePath : instancePaths) {
            ...
            for (FairRunConfig config : configs) {
                for (int algorithmIndex = 0; algorithmIndex < algorithms.size(); algorithmIndex++) {
                    FairAlgorithmDefinition definition = algorithms.get(algorithmIndex);
    ...
    private static void printHeader() {
        System.out.println("instance,runIndex,startVertexId,runSeed,algorithmIndex,algorithm,objective,profitSum,tourLength,cycleSize,runtimeNanos,iterationCount,acceptedBetterCount,acceptedWorseCount,rejectedWorseCount,bestFoundIteration,timeLimitSeconds");
    }
